...
public:
    ...
    unordered_set<int> generatePossiblePos(int x, vector<int>& fences) {
        ...
        for(int i = 0; i < fences.size(); i++) for(int j = i + 1; j < fences.size(); j++) possible.insert(fences[j] - fences[i]);
        return possible;
    }
    ...
    int maximizeSquareArea(int n, int m, vector<int>& hFences, vector<int>& vFences) {
        ...
        long long area = -1;
        for(auto& x : possibleH) if(possibleW.find(x) != possibleW.end()) if(area < x) area = x;
